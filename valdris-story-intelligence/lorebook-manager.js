...
{{#if notes}}Notes: {{notes}}{{/if}}`,
    ...
    generateEntryContent(entity) {
        ...
        const data = this.buildTemplateData(entity);
        return this.fillTemplate(template.format, data);
    }
    ...
    generateEntryName(entity) {
        ...
        if (settings.includeTimestamp && entity.createdAt) {
            const date = new Date(entity.createdAt).toLocaleDateString();
            name = `${name} • ${date}`;
        }
    ...
    buildTemplateData(entity) {
        if (entity.type !== 'npc') {
            return entity;
        }

        const appearanceParts = [];
        if (entity.appearance?.height) appearanceParts.push(entity.appearance.height);
        if (entity.appearance?.build) appearanceParts.push(entity.appearance.build);
        if (entity.appearance?.hairColor) appearanceParts.push(`${entity.appearance.hairColor} hair`);
        if (entity.appearance?.eyeColor) appearanceParts.push(`${entity.appearance.eyeColor} eyes`);

        const personalityTraits = entity.personality?.traits?.join(', ') || '';
        const relationship = entity.playerRelationship?.type || entity.relationship;
        const locationMet = entity.playerRelationship?.firstMet?.location || entity.location_met;
        const dateMet = entity.playerRelationship?.firstMet?.date || '';
        const occupation = entity.background?.occupation || entity.occupation || '';
