...
export class EntityDatabase {
    ...
    addEntity(type, entityData) {
        ...
        this.listeners.added.forEach((callback) => callback(entity));
        return entity;
    }
    ...
    updateEntity(type, id, updates) {
        ...
        this.stateManager.saveState();
        const entity = section[id];
        this.listeners.updated.forEach((callback) => callback(entity));
        return entity;
    }
    ...
    deleteEntity(type, id) {
        ...
        delete section[id];
        this.stateManager.saveState();
        this.listeners.deleted.forEach((callback) => callback(entity));
        return true;
    }
    ...
    getEntityById(entityId) {
        const entities = this.stateManager.getSection('entities');
        for (const section of Object.values(entities)) {
            if (section[entityId]) {
    ...
    onEntityUpdated(callback) {
        this.listeners.updated.push(callback);
    }

    onEntityDeleted(callback) {
        this.listeners.deleted.push(callback);
    }
